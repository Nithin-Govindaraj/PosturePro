...
class FallStateMachine:
    ...
    def __init__(
        ...
        self._RECOVERY_NEEDED  = 3

        # Strong fall: if 3 signals fire at once, skip voting window delay
        self._STRONG_FALL_SIGNALS = 3
    ...
    def _handle_monitoring(self, kp):
        ...
        self._no_person_frames = 0
        vote, signal_count = self._classify_fall(kp)
        self._vote_window.append(vote)

        fall_votes = sum(self._vote_window)
        log.debug(f"Vote: {'FALL' if vote else 'OK'} | Window: {fall_votes}/{len(self._vote_window)} | Signals: {signal_count}")

        # Fast path: if all 4 signals fire, start confirmation timer immediately
        # without waiting for full voting window to fill
        strong_fall = signal_count >= self._STRONG_FALL_SIGNALS

        window_full_and_voting = (
            len(self._vote_window) == VOTE_WINDOW_SIZE and
            fall_votes >= VOTE_THRESHOLD
        )

        if strong_fall or window_full_and_voting:
            ...
            if elapsed >= FALL_CONFIRM_SECONDS:
                self._transition_to_fall_detected()
        else:
            # Only reset timer if signals clearly gone (not just borderline)
            if signal_count == 0:
                self._fall_start_time = None
            elif not vote and self._fall_start_time is not None:
                # Weak non-fall: give small grace before resetting timer
                # This prevents timer reset on single bad frame
                pass
    ...
    def _handle_fall_detected(self, kp):
        ...
        if self.voice_cancel_received:
            log.info(f"Voice cancel at {elapsed:.1f}s -> recovering")
            self.voice_cancel_received = False
            self.on_recover()
            self._transition_to_monitoring()
            return
        ...
        if kp is not None:
            ...
            if not vote:
                ...
                if self._recovery_frames >= self._RECOVERY_NEEDED:
                    log.info(f"Pose recovered at {elapsed:.1f}s -> MONITORING")
                    self._recovery_frames = 0
                    self.on_recover()
                    self._transition_to_monitoring()
                    return
    ...
    def _handle_sos_sent(self, kp):
        ...
        if kp is not None:
            ...
            if not vote:
                self._recovery_frames += 1
                if self._recovery_frames >= self._RECOVERY_NEEDED:
                    log.info("Pose recovered after SOS -> cancel SMS")
                    self._recovery_frames = 0
                    self.on_sos_cancel()
                    self._transition_to_monitoring()
                    return
    ...
    def _transition_to_fall_detected(self):
        self.state            = State.FALL_DETECTED
        self._beep_start_time = time.time()
        self._sos_sent        = False
        self._recovery_frames = 0
        self._vote_window.clear()
        log.alert(f"FALL CONFIRMED - beep starting, {SOS_TIMER_SECONDS}s SOS timer")
    ...
    def _classify_fall(self, kp: PoseKeypoints):
        # Returns (vote: bool, signal_count: int)
        signals = []
        ...
        if kp.sar is not None:
            signals.append(kp.sar < SAR_FALL_THRESHOLD)

        hip_angles = [a for a in [kp.left_hip_angle, kp.right_hip_angle] if a is not None]
        if hip_angles:
            avg_hip = sum(hip_angles) / len(hip_angles)
            signals.append(avg_hip > HIP_ANGLE_FALL_THRESHOLD)

        if not signals:
            return False, 0

        fall_votes   = sum(signals)
        vote_result  = fall_votes >= 2
        log.debug(f"Classifier: {fall_votes}/{len(signals)} -> {'FALL' if vote_result else 'OK'}")
        return vote_result, fall_votes
