# main.py - PosturePro Fall Detection System (Laptop Version)
# Run: python main.py
# Stop: Q key or Ctrl+C
...
from config import (
    ...
    MP_MODEL_COMPLEXITY, MP_MIN_DETECTION_CONFIDENCE,
    MP_MIN_TRACKING_CONFIDENCE, FALL_CONFIRM_SECONDS,
    SOS_TIMER_SECONDS, KEYPOINT_VISIBILITY_THRESHOLD,
)
...
def main():
    global _running
    log.info("PosturePro Fall Detection (Laptop Version) - Starting")
    ...
    while _running:
        ...
        if voice_listener and voice_listener.is_available:
            ...
            draw_text_bg(frame, "MIC: ON" if mic_on else "MIC: OFF",
                         (FRAME_WIDTH - 95, 55), scale=0.55, color=mic_col)

        cv2.imshow("PosturePro Fall Detection [Q to quit]", frame)
