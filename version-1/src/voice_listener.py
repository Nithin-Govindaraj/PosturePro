# voice_listener.py - Vosk keyword detection (Laptop Version)
...
class VoiceListener:
    ...
    def _check_ok_with_confidence(self, result, text):
        for phrase in OK_PHRASES:
            ...
            if words:
                ...
                if confidences:
                    ...
                    if avg_conf >= VOICE_CONFIDENCE_THRESHOLD:
                        log.info(f"OK confirmed (conf={avg_conf:.2f}): '{text}'")
                        self.state_machine.voice_cancel_received = True
                        return
            else:
                log.info(f"OK matched: '{text}'")
                self.state_machine.voice_cancel_received = True
                return
